...
If Playwright is newly installed, run `npx playwright install` once. If `npm install` fails on `/mnt/<drive>/...` in WSL, move the repo into `~/...` first (mounted drives can cause `EPERM`/`chmod` errors).
